...
export default function BingMapsReact({
  ...
  // add polygons
  const addPolygons = useCallback((polygonsToAdd, map, Maps) => {
    
    polygonsToAdd.forEach((polygon) => {
        if (polygon === null) {
            return;
        }
        const newPolygon = new Maps.Polygon(polygon.coords, polygon.options);
  ...
  const makeMap = useCallback(() => {
    ...
    if (polygons) {
        addPolygons(polygons, map.current, Maps);
    }
